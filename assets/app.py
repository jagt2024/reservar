...
def page_vehicle_detail():
    ...
    with col_m:
        # ══════════════════════════════════════════════════════════════════════
        # GALERÍA DE IMÁGENES
        # Prioridad: data_uri/preview_url en memoria → fotos_urls CSV (Drive/b64/local)
        # ══════════════════════════════════════════════════════════════════════
        img_items = []  # cada item es str (data URI o URL pública) o PIL Image

        # Prioridad 1: fotos ya en memoria (data_uri, preview_url o bytes)
        for fi in (fotos or []):
            if not isinstance(fi, dict):
                continue
            # data_uri cubre tanto data:image/... como URLs thumbnail de Drive
            uri = fi.get("data_uri") or fi.get("preview_url") or ""
            if uri:
                img_items.append(uri)
                continue
            # bytes en RAM (publicación recién subida, sin data_uri aún)
            if fi.get("bytes"):
                try:
        ...
        # Prioridad 2: si no se cargaron fotos aún, reconstruir desde fotos_urls
        # (cubre el caso de recarga de página donde fotos[] llegó vacío de Sheets)
        if not img_items:
            fotos_csv = (v.get("fotos_urls") or "").strip()
            if fotos_csv:
                ...
                for ref in [r.strip() for r in fotos_csv.split(",") if r.strip()]:
                    if _is_drive_ref(ref):
                        ...
                        img_items.append(_thumbnail_url_drive(fid, size=800))
                        ...
                        raw = _leer_local(ref)
                        if raw:
                            img_items.append(_a_data_uri(raw, 800))

        def _render_img(src, style="width:100%;border-radius:10px;"):
            if isinstance(src, str):  # data URI o URL pública
        ...
        # ══════════════════════════════════════════════════════════════════════
        # VIDEO
        # Prioridad: bytes en RAM → embed_url (Drive) → gdrive: en video_url
        # ══════════════════════════════════════════════════════════════════════
        video_mostrado = False

        # 1. Bytes en memoria (publicación recién subida en esta sesión)
        video_bytes = video.get("bytes") if isinstance(video, dict) else None

        # 2. embed_url ya construido (tras recarga desde Sheets)
        embed_url = video.get("embed_url") if isinstance(video, dict) else None

        # 3. Si no hay ninguna de las anteriores, intentar construir desde video_url
        if not video_bytes and not embed_url:
            vref = (v.get("video_url") or "").strip()
            if vref:
                from media_sync import _is_drive_ref, _file_id_from_ref, _leer_local
