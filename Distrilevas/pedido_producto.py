...
logging.basicConfig(level=logging.DEBUG, filename='pedido_producto.log', filemode='w',
...
def crea_reserva():
  ...
  try:
    ...
    st.title('Generar Pedido del Producto')
    ...
    try:
        ...
        with col1:
            ...
            nombre = st.text_input('Nombre Solicitante*: ', placeholder='Nombre', key='nombre_p',value=st.session_state.nombre
            ...
            fecha  = st.date_input('Fecha Servicio*: ', key='fecha_p')
            notas = st.text_area('Nota de Consulta u Observacion(Opcional)', key='notas_p'
                                 ,value=st.session_state.notas
                                 )
        with col2:

            email  = st.text_input('Email Solicitante:', placeholder='Email', key='email_p',value=st.session_state.email
            )
            # Selector de producto
            producto_seleccionado = st.selectbox(
                'Seleccione el producto:',
                producto,
                key='producto_selector_p'
            )
            direccion = st.text_input('Direccion Ubicacion solicitante :', placeholder='Direccion', key='direccion_p',value=st.session_state.direccion)  
            ...
            if encargado:
                conductor_seleccionado = st.selectbox(
                    'Encargado Entrega:',
                    encargado,
                    key='conductor_selector_p'
                )
                
            
            hora = st.selectbox('Hora Servicio: ', horas, key="hora_new_p")
            ...
            whatsapp = st.checkbox('Envio a WhatsApp Si/No (Opcional)', key='whats_p')
            telefono = st.text_input('Nro. Telefono', key='telefono_p',value=st.session_state.telefono)
    ...
    with st.form(key='myform00',clear_on_submit=True):
