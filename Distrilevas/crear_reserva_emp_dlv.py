...
logging.basicConfig(level=logging.DEBUG, filename='crear_reserva_emp_dlv.log', filemode='w',
  ...
  for intento in range(MAX_RETRIES):
    try:
      with st.spinner(f'Cargando datos... (Intento {intento + 1}/{MAX_RETRIES})'):
        ...
        workbook = gc.open('gestion-reservas-dlv')
      ...
      #with st.spinner(f'Cargando datos... (Intento {intento + 1}/{MAX_RETRIES})'):
        ...
        with col1:
          #for intento in range(MAX_RETRIES):
            ...
            if servicio_seleccionado in ['Programar Pedido','Entrega Pedido','Cambio o Devolucion Producto']:
        ...
        with st.form(key='myform0', clear_on_submit=True):
            ...
            if enviar:
                with st.spinner('Cargando...'):
                 ...
                 if servicio_seleccionado in ['Programar Pedido', 'Entrega Pedido']:
                    # Validaciones
                    if  not email or not direccion:
                        ...
                        if selected_option == '-- Añadir Nuevo Cliente --':        
                          ...
                          try:
                            ...
                            gs = GoogleSheet(st.secrets['sheetsemp']['credentials_sheet'], 'gestion-reservas-dlv', 'clientes')
                            ...
                            values = [(
                                ...
                                f"web.whatsapp.com/send?phone=&text=Reserva para { nuevo_nombre}", 
                                '=ArrayFormula(SI(M3=VERDADERO;HIPERVINCULO(O3;"Enviar");"No Enviar")), "Confirmado"'
                            )]

                            # Guardar en Google Sheets
                            gs = GoogleSheet(st.secrets['sheetsemp']['credentials_sheet'], 'gestion-reservas-dlv', 'reservas')
                            range2 = gs.get_last_row_range()
                            gs.write_data(range2, values)

                            st.success('Su solicitud ha sido recibida de forma exitosa, la confirmación fue enviada al correo')
                            ...
                            if whatsapp == True or whatsapp == 'Verdadero':
                                ...
                                message = f'Cordial saludo: Sr(a): {nuevo_nombre} Su solicituud de pedido se creó con éxito para el día: {fecha} a las: {hora} con el encargado: {conductor_seleccionado} para el servicio: {servicio_seleccionado}. Productos: {productos_str}. Cordialmente, aplicación de Solicitudes y Despachos.'
                            ...
                            try:           
                                ...
                                if whatsapp == True or whatsapp == 'Verdadero':
                                    contact = str(57)+telefono
                                    message = f'Cordial saludo: Sr(a): { selected_option} Su solicituud de pedido se creó con éxito para el día: {fecha} a las: {hora} con el encargado: {conductor_seleccionado} para el servicio: {servicio_seleccionado}. Productos: {productos_str}. Cordialmente, aplicación de Solicitudes y Despachos.'
                 ...
                 elif servicio_seleccionado == 'Cambio o Devolucion Producto':
                    ...
                    try:
                      if selected_option != '-- Añadir Nuevo Cliente --':    
                        ...
                        values4 = [(
                            selected_option, email, str(fecha), hora, servicio_seleccionado, 
                            conductor_seleccionado, zona_seleccionada, productos_str, notas
                            )]
                                                  
                        # Guardar en Google Sheets
                        gs = GoogleSheet(st.secrets['sheetsemp']['credentials_sheet'], 'gestion-reservas-dlv', 'devoluciones')
                        range2 = gs.get_last_row_range()
                        gs.write_data(range2, values4)
                        ...
                        if whatsapp == True or whatsapp == 'Verdadero':
                           contact = str(57)+telefono
                           message = f'Cordial saludo: Sr(a): {selected_option} Su solicituud de cambio o deevolucion se creó con éxito para el día: {fecha} a las: {hora} con el encargado: {conductor_seleccionado} para el servicio: {servicio_seleccionado}. Productos: {productos_str}. Cordialmente, aplicación de Solicitudes y Despachos.'
                 ...
                 elif servicio_seleccionado == 'Consulta en General':
                    if selected_option == '-- Añadir Nuevo Cliente --':
