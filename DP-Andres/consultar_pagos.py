...
PAGOS_COLUMNS = [
    'Fecha_Pago', 'Nombre', 'Email', 'Fecha_Servicio', 'Hora_Servicio',
    'Servicio', 'Valor', 'Estado_Pago', 'Referencia_Pago', 'Encargado',
    ...
    if df is not None:
        ...
        with col1:
            servicios = st.multiselect(
                'Servicios',
                options=sorted(df['Servicio'].unique())
            )
        ...
        if servicios:
            mask &= df['Servicio'].isin(servicios)
        if estados:
        ...
        if not filtered_df.empty:
            ...
            with met1:
                st.metric("Total Servicios", len(filtered_df['Servicio'].unique()))
            with met2:
                st.metric("Total Pagos", len(filtered_df))
            with met3:
                st.metric("Valor Total", f"${filtered_df['Valor_Pagado'].sum():,.2f}")
            with met4:
                st.metric("Promedio por Servicio", f"${filtered_df['Valor_Pagado'].mean():,.2f}")
