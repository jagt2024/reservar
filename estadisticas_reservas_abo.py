...
def main_reservas_abo():
    ...
    try:
        ...
        if st.button("Aplicar Filtro"):
            ...
            fig = px.bar(services_count, x=services_count.index, y=services_count.values, labels={'x': 'Servicio', 'y': 'Cantidad'})
            st.plotly_chart(fig)
            ...
            encargado_select = st.selectbox("Seleccione un Encargado", options=['Todos'] + list(filtered_df['ENCARGADO'].unique()))
            ...
            if encargado_select == 'Todos':
                ...
                fig = px.bar(top_services, x=top_services.index, y=top_services.values,  labels={'x': 'Servicio', 'y': 'Cantidad'})
                st.plotly_chart(fig)
