...
def mostrar_ongrid(proyecto_id: int, session_state: dict) -> None:
    ...
    cargas_df = pd.read_sql("SELECT cantidad, potencia_w, horas_dia FROM cargas WHERE proyecto_id=?",
    ...
    with tab_og1:
        ...
        if consumo_inv == 0 and consumo_rec == 0:
            st.markdown("""
            <div class='warn-box'>⚠ No hay consumo registrado. Ve al módulo <b>Dimensionamiento OFF-GRID</b>
            → Tab 1 (Cargas) o Tab 2 (Recibo) para ingresar el consumo del proyecto primero.</div>
            """, unsafe_allow_html=True)
            return
    ...
    with tab_og2:
        ...
        with col_hsp2:
            irr_og = st.number_input(
                ...
                help="Valor promedio anual de irradiación mensual para tu ubicación",
                key="og_irr")
            hsp_og = irr_og / 30
    ...
    with tab_og5:
        ...
        with col_e1:
            ...
            tarifa_kwh = st.number_input(
                ...
                value=700.0, step=50.0,
                help="Tarifa promedio de energía eléctrica (Colombia: ~$600–$900/kWh)", key="og_tarifa")
