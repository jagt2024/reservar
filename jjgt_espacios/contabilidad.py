...
# ══════════════════════════════════════════════════════════════════════════════
#
#  Instalación adicional (ya incluida en pagos_convenios.py):
#    pip install openpyxl pandas
#
#  Uso: importar en pagos_convenios.py y llamar desde los eventos de negocio.
#
#  Hoja de ruta de integración:
#    from contabilidad import (
#        comp_ingreso_pago,
#        comp_ingreso_factura,
#        comp_egreso_proveedor,
#        comp_diario_ajuste,
#        comp_nomina,
#        exportar_libro_contable,
#    )
#
#  Los comprobantes se guardan en Google Sheets (hoja "Comprobantes_Contables")
#  y opcionalmente se exportan a Excel descargable desde el panel operador.
#
#  Plan de Cuentas PUC Colombia (subset usado):
#    110505 → Caja General
#    111005 → Banco (Caja Social)
#    130505 → Clientes nacionales
#    220505 → Proveedores nacionales
#    236505 → IVA generado
#    236805 → Retención en la fuente por pagar
#    410510 → Ingresos por servicios de alojamiento
#    511010 → Gastos de personal – sueldos
#    511505 → Gastos de personal – prestaciones sociales
#    519515 → Otros gastos operacionales
#    516010 → Depreciación propiedades planta y equipo
...
# Nombre de la hoja en Google Sheets donde se almacenan los comprobantes
...
_PREFIJOS = {
    ...
    "diario":   "CD",   # Comprobante Diario / Ajuste
    "nomina":   "CN",   # Comprobante de Nómina
    "factura":  "CF",   # Comprobante de Factura (causación)
}
...
_COLUMNAS = [
    "Fecha",
    "Numero",
    "Tipo",           # ingreso | egreso | diario | nomina | factura
    "Evento",         # nueva_reserva | pago_proveedor | ajuste | nomina | ...
    "Tercero",
    "Descripcion",
    "Cuenta_Debito",
    "Nombre_Debito",
    ...
    "Valor_COP",
    "Medio_Pago",
    "Soporte",        # número de reserva, factura, etc.
    "Operador",
    "Observaciones",
]

# Catálogo de cuentas PUC (código → nombre)
...
# Reglas contables automáticas por evento
# Formato: evento → (cuenta_debito, cuenta_credito)
REGLAS_CONTABLES: dict[str, tuple[str, str]] = {
    "nueva_reserva":    ("130505", "410510"),  # Clientes → Ingresos alojamiento
    "pago_efectivo":    ("110505", "130505"),  # Caja     → Clientes
    "pago_banco":       ("111005", "130505"),  # Banco    → Clientes
    "pago_digital":     ("111005", "130505"),  # Banco    → Clientes (Nequi/PSE…)
    "pago_convenio":    ("130505", "410510"),  # Clientes → Ingresos (pendiente cobro)
    "pago_proveedor":   ("220505", "111005"),  # Proveedores → Banco
    "iva_ventas":       ("410510", "236505"),  # Ingresos → IVA x pagar
    "nomina_causacion": ("511010", "282005"),  # Gasto nómina → Nómina x pagar
    "nomina_pago":      ("282005", "111005"),  # Nómina x pagar → Banco
    "depreciacion":     ("516010", "159210"),  # Gasto deprec. → Deprec. Acumulada
    "ajuste_diario":    ("519515", "111005"),  # Otros gastos → Banco
}

# Contexto inyectado desde pagos_convenios.py (get_active_client, _gs_append, etc.)
_ctx: dict = {}
_ctx_lock = threading.Lock()


# ══════════════════════════════════════════════════════════════════════════════
# API DE CONTEXTO — llamada desde pagos_convenios.py
# ══════════════════════════════════════════════════════════════════════════════

def set_context(globals_dict: dict) -> None:
    """
    Inyecta el contexto de pagos_convenios.py para poder escribir en
    Google Sheets usando las funciones ya existentes.
...
def _consecutivo(tipo: str) -> str:
    """
    Genera el número correlativo del comprobante leyendo la hoja en Sheets.
    Formato: CI-0001, CE-0002, CD-0003 …
    """
...
def _escribir_comprobante(fila: list) -> None:
    """Escribe una fila en la hoja Comprobantes_Contables de Google Sheets."""
    try:
        fn_append      = _ctx.get("_gs_append")
        fn_get_client  = _ctx.get("get_active_client")
        if not fn_append or not fn_get_client:
            print(f"[contabilidad] WARN: contexto incompleto — comprobante no guardado: {fila[1] if len(fila) > 1 else '?'}")
            return
        _, sh = fn_get_client()          # ← obtener el spreadsheet activo
        if not sh:
            print("[contabilidad] WARN: sin spreadsheet activo — comprobante no guardado.")
            return
        fn_append(sh, GS_HOJA_COMP, fila)   # ← pasar sh en vez de None
        fn_inv = _ctx.get("_gs_invalidate_cache")
        if fn_inv:
            fn_inv(GS_HOJA_COMP)
    except Exception as e:
        print(f"[contabilidad] ERROR al escribir comprobante: {e}")


def _operador_actual() -> str:
    """Intenta leer el nombre del operador activo desde session_state."""
    try:
...
def _medio_a_evento(metodo_pago: str) -> str:
    """Convierte método de pago → clave de regla contable."""
    m = (metodo_pago or "").lower()
    if m == "efectivo":
        return "pago_efectivo"
    if m == "convenio":
        return "pago_convenio"
    # Nequi, Daviplata, PSE, MercadoPago, Transferencia, Tarjeta → banco/digital
    return "pago_banco"
...
def comp_ingreso_pago(
    ...
    Uso en pagos_convenios.py — dentro de crear_reserva_completa():
        from contabilidad import comp_ingreso_pago
        comp_ingreso_pago(
            tercero    = cliente["nombre"],
            valor      = calc["total"],
            metodo_pago= metodo,
            soporte    = num_res,
            descripcion= f"Pago reserva {num_res} · {metodo}",
        )
    """
    tipo   = "ingreso"
    evento = _medio_a_evento(metodo_pago)
    numero = _consecutivo(tipo)
...
def comp_ingreso_factura(
    ...
    """
    Causación contable al emitir una factura de venta.

    Llama automáticamente dos líneas:
    1) Clientes  (130505) → Ingresos alojamiento (410510) por el subtotal
    2) Ingresos  (410510) → IVA generado x pagar (236505) por el IVA

    Uso en registrar_en_facturacion() de pagos_convenios.py:
        from contabilidad import comp_ingreso_factura
        comp_ingreso_factura(
            tercero       = cliente["nombre"],
            valor_subtotal= reserva["subtotal"],
            valor_iva     = reserva["iva"],
            num_factura   = numf,
        )
    """
    tipo   = "factura"
    ...
    _escribir_comprobante(fila1)

    # Línea 2: IVA (solo si aplica)
...
def comp_egreso_proveedor(
    ...
    Uso desde el panel operador (compras, servicios, arreglos):
        from contabilidad import comp_egreso_proveedor
        comp_egreso_proveedor(
            beneficiario = "Proveedor Aseo SAS",
            valor        = 150000,
            descripcion  = "Insumos de limpieza",
            medio_pago   = "Transferencia",
            soporte      = "FAC-2025-001",
        )
    """
    tipo   = "egreso"
    numero = _consecutivo(tipo)
    deb, cred = cuenta_gasto, "111005"  # Gasto → Banco
...
def comp_diario_ajuste(
    ...
    """
    Comprobante diario para ajustes manuales (depreciaciones, diferidos, errores).

    Uso:
        from contabilidad import comp_diario_ajuste
        comp_diario_ajuste(
            descripcion    = "Depreciación equipos mes de mayo",
            cuenta_debito  = "516010",
            cuenta_credito = "159210",
            valor          = 250000,
            soporte        = "Ajuste contable mayo-2025",
        )
    """
    tipo   = "diario"
    numero = _consecutivo(tipo)
...
def comp_nomina(
    ...
    """
    Comprobante de nómina en dos pasos:
    1) Causación:  Gasto nómina (511010) → Nómina x pagar (282005)
    2) Pago:       Nómina x pagar (282005) → Banco (111005)

    Uso:
        from contabilidad import comp_nomina
        comp_nomina(
            periodo            = "Mayo 2025",
            total_sueldos      = 2_800_000,
            total_prestaciones = 700_000,
            total_pagado       = 3_500_000,
        )
    """
    tipo   = "nomina"
    numero = _consecutivo(tipo)

    # Línea 1: causación sueldos
    if total_sueldos > 0:
        fila1 = [
    ...
    # Línea 2: causación prestaciones
    ...
    # Línea 3: pago efectivo de nómina
    if total_pagado > 0:
        num3 = f"{numero}-PAGO"
        fila3 = [
        ...
        _escribir_comprobante(fila3)

    return numero


# ══════════════════════════════════════════════════════════════════════════════
# INTEGRACIÓN DIRECTA CON EVENTOS DE pagos_convenios.py
# ══════════════════════════════════════════════════════════════════════════════

def on_reserva_creada(voucher: dict, calc: dict, cliente: dict, metodo: str) -> None:
    """
    Hook que se llama automáticamente al final de crear_reserva_completa().
    Genera el comprobante de ingreso + causación de factura en un solo paso.

    Agregar al final de crear_reserva_completa() en pagos_convenios.py:
        try:
            import contabilidad
            contabilidad.on_reserva_creada(voucher, calc, cliente, metodo)
        except Exception:
            pass
    """
    try:
...
def on_pago_convenio(
    ...
    """
    Hook para pagos bajo convenio empresarial (cobro diferido).
    Registra la causación de la deuda del convenio.

    Agregar en el bloque de cierre del TAB Convenio de pagos_convenios.py:
        try:
            import contabilidad
            contabilidad.on_pago_convenio(empresa, valor, num_reserva, num_factura)
        except Exception:
            pass
    """
    try:
...
# ══════════════════════════════════════════════════════════════════════════════
# EXPORTACIÓN A EXCEL (descargable desde el panel operador)
# ══════════════════════════════════════════════════════════════════════════════
...
def exportar_libro_contable(
    ...
    """
    Lee todos los comprobantes de Google Sheets y genera un libro Excel
    descargable con:
      - Hoja "Comprobantes": listado completo con formato profesional
      - Hoja "Resumen": totales por tipo de comprobante y por cuenta
      - Hoja "Balance_Cuentas": saldo de cada cuenta en el período

    Retorna bytes del .xlsx o None si no hay datos / openpyxl no disponible.

    Uso en el panel operador (sidebar "Contabilidad"):
        bytes_excel = exportar_libro_contable("2025-05-01", "2025-05-31")
        if bytes_excel:
            st.download_button("📥 Descargar libro contable", bytes_excel,
                               "libro_contable.xlsx",
    ...
    try:
        ...
        if fn_read and fn_val:
            ...
            for r in rows:
                ...
                # Filtro de fechas si se suministra rango
    ...
    # ── Paleta de colores Suite Salitre ──────────────────────────────────────
    C_AZUL_OSC  = "050B1A"
    C_AZUL_CARD = "0D1F3C"
    C_CYAN      = "00D4FF"
    C_VERDE     = "00FF88"
    ...
    ws1["A1"] = "SUITE SALITRE · Libro de Comprobantes Contables"
    ...
    hdr_res = ["Tipo", "Cantidad", "Total COP", "% del Total"]
    for col, h in enumerate(hdr_res, start=1):
    ...
    ws3["A1"] = "Balance de Cuentas del Período"
    ws3["A1"].font = _font(bold=True, color=C_CYAN, size=12)
    ws3["A1"].fill = _fill(C_AZUL_OSC)
    ws3["A1"].alignment = _center()
    ws3.row_dimensions[1].height = 26

    hdr_bal = ["Cuenta", "Nombre", "Total Débitos COP", "Total Créditos COP", "Saldo COP"]
    for col, h in enumerate(hdr_bal, start=1):
...
# ══════════════════════════════════════════════════════════════════════════════
# PANEL STREAMLIT (widget embebible en show_operador de pagos_convenios.py)
# ══════════════════════════════════════════════════════════════════════════════

def render_panel_contabilidad() -> None:
    """
    Renderiza el panel de contabilidad dentro del panel operador.

    Agregar en show_operador() de pagos_convenios.py como nueva opción del
    menú lateral:
    ...
    st.caption(
        "Todos los movimientos del sistema generan su comprobante automáticamente. "
        "Aquí puedes consultarlos y exportar el libro contable en Excel."
    )
    ...
    with tab1:
        try:
            if fn_read and fn_val and fn_float:
                ...
                if not rows_comp:
                    st.info("No hay comprobantes registrados aún. Se generarán automáticamente al procesar reservas y pagos.")
                else:
                    import pandas as pd
                    data = []
                    for r in rows_comp:
                ...
                st.warning("Contexto no inyectado. Llama `contabilidad.set_context(globals())` en `main()`.")
        except Exception as e:
            st.error(f"Error al leer comprobantes: {e}")

    # ── TAB 2: Comprobante manual ────────────────────────────────────────────
    with tab2:
        st.markdown("Registra un comprobante contable manual (ajuste, gasto, nómina).")
        ...
        if tipo_man == "Egreso / Pago proveedor":
            ...
            with col2:
                ...
                cta_g   = st.text_input("Cuenta de gasto (PUC)", value="519515", key="cont_cta_g",
                                        help="Ej: 519515 = Otros gastos. Ver Plan de Cuentas.")
            ...
            if st.button("💾 Registrar egreso", key="btn_cont_egreso", type="primary"):
                if not ben or not val_e:
                    ...
                    num = comp_egreso_proveedor(ben, float(val_e), desc_e, medio_e, sop_e, cta_g)
                    st.success(f"✅ Comprobante de egreso registrado: **{num}**")
            ...
            if st.button("💾 Registrar ajuste", key="btn_cont_diario", type="primary"):
                if not desc_d or not val_d:
                    ...
                    num = comp_diario_ajuste(desc_d, cta_deb, cta_cre, float(val_d), sop_d)
                    st.success(f"✅ Comprobante diario registrado: **{num}**")
            ...
            if st.button("💾 Registrar nómina", key="btn_cont_nomina", type="primary"):
                if not periodo:
                    ...
                    num = comp_nomina(periodo, float(sueldos), float(presta), float(pagado))
                    st.success(f"✅ Nómina registrada: **{num}**")
    ...
    with tab3:
        st.markdown("Exporta el libro contable del período a Excel (.xlsx).")
        now_col = fn_ahora()
        ...
        if st.button("⚙️ Generar libro contable", key="btn_cont_export", type="primary",
                     use_container_width=True):
            with st.spinner("Generando libro Excel…"):
                xlsx_bytes = exportar_libro_contable(str(fi), str(ff))
            if xlsx_bytes:
                nombre_arch = (
                    f"libro_contable_SuiteSalitre_{fi.strftime('%Y%m%d')}"
                    f"_{ff.strftime('%Y%m%d')}.xlsx"
                )
                st.download_button(
                ...
                st.success(
                    ...
                    f"Contiene hojas: Comprobantes, Resumen, Balance_Cuentas"
                )
            else:
                st.warning(
                    "No hay comprobantes en ese período o falta la librería openpyxl. "
