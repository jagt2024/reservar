...
def load_excel_data(file):
    try:
        ...
        required_columns = ['TELEFONO', 'ACCION', 'WHATSAPP','PROCESO', 'FECHA']
        if not all(col in df.columns for col in required_columns):
            st.error("El archivo Excel debe contener las columnas 'TELEFONO', 'ACCION', 'WHATSAPP', 'PROCESO' y 'FECHA'")
            return None
        return df
        ...
        if df is not None:
            ...
            if df_filtered.empty:
                ...
                for index, row in df_filtered.iterrows():
                    if st.checkbox(f"{row['PROCESO']} - {row['TELEFONO']} - {str(row['ACCION'])[:50]}... - Fecha: {row['FECHA'].strftime('%Y-%m-%d')}", key=f"checkbox_{index}"):
                ...
                if st.button("Generar Enlaces de WhatsApp"):
                    ...
                    if not df_to_send.empty:
                        ...
                        for _, row in df_to_send.iterrows():
                            ...
                            message = str(row['ACCION'])
                            whatsapp_link = generate_whatsapp_link(phone_number, message)
                            st.markdown(f"[Enviar mensaje a {row['PROCESO']}]({whatsapp_link})")
                        
                        # Preparar DataFrame para descargar
                        df_to_send['ENLACE_WHATSAPP'] = df_to_send.apply(lambda row: generate_whatsapp_link(format_phone_number(row['TELEFONO']), str(row['ACCION'])), axis=1)
